-- SIMPLE PAYROLL CALCULATION FUNCTION
-- Versi simple tanpa dependencies yang kompleks

-- ========================================
-- SIMPLE VERSION OF PAYROLL CALCULATION
-- ========================================
...
BEGIN
  BEGIN
    WITH advance_summary AS (
      ...
      LEFT JOIN public.advance_repayments ar ON ar.advance_id = ea.id
      WHERE ea.employee_id = emp_id
        AND ea.date <= up_to_date
    )
    SELECT GREATEST(total_advances - total_repayments, 0)
    INTO result
    FROM advance_summary;
  EXCEPTION WHEN OTHERS THEN
    result := 0; -- Default if tables don't exist
  END;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Simple payroll calculation function created!';
  RAISE NOTICE '🔧 Handles missing tables gracefully';
  RAISE NOTICE '📊 Returns error info in JSON instead of throwing exceptions';
