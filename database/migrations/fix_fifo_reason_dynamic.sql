-- ============================================================================
-- FIX FIFO REASON DYNAMIC (Remove Hardcoded Status)
-- Mengubah consume_material_fifo agar menerima p_reason secara dinamis,
-- dan menghapus hardcoded 'SPOILAGE' statuse.
-- ============================================================================

-- 1. DROP OLD FUNCTION (to avoid overload confusion)
DROP FUNCTION IF EXISTS consume_material_fifo(UUID, UUID, NUMERIC, TEXT, TEXT);

-- 2. CREATE NEW CONSUME_MATERIAL_FIFO (with p_reason)
CREATE OR REPLACE FUNCTION consume_material_fifo(
  p_material_id UUID,
  p_branch_id UUID,
  p_quantity NUMERIC,
  p_reference_id TEXT DEFAULT NULL,
  p_reference_type TEXT DEFAULT 'production', -- 'production', 'adjustment', 'transaction'
  p_reason TEXT DEFAULT 'OUT'                 -- 'PRODUCTION', 'ADJUSTMENT', 'SALES', etc.
)
RETURNS TABLE (
  success BOOLEAN,
  total_cost NUMERIC,
...
-- 3. UPDATE PROCESS_PRODUCTION_ATOMIC (Use reason 'PRODUCTION')
...
-- 4. UPDATE PROCESS_SPOILAGE_ATOMIC (Use reason 'ADJUSTMENT' and type 'adjustment')
