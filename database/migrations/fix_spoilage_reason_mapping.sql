-- ============================================================================
-- Fix: Update consume_material_fifo to map 'spoilage' to 'PRODUCTION_ERROR'
-- Run this on VPS database (aquvit_dev)
-- ============================================================================

-- Drop existing function to update signature
DROP FUNCTION IF EXISTS consume_material_fifo(UUID, UUID, NUMERIC, TEXT, TEXT);
DROP FUNCTION IF EXISTS consume_material_fifo(UUID, UUID, NUMERIC, TEXT, TEXT, TEXT);
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      INSERT INTO inventory_batch_consumptions (
        ...
        v_batch.id, v_deduct_qty, NOW(), p_reference_id, p_reference_type,
        COALESCE(v_batch.unit_cost, 0), v_deduct_qty * COALESCE(v_batch.unit_cost, 0)
      );
  ...
  -- Log to material_stock_movements with FIXED reason mapping
  INSERT INTO material_stock_movements (
    ...
    CASE
      ...
      WHEN p_reference_type = 'spoilage' THEN 'PRODUCTION_ERROR'  -- FIXED: Map spoilage to valid constraint value
      ELSE 'ADJUSTMENT'
    END,
    p_quantity,
    v_available_stock,
...
-- Verify
SELECT 'consume_material_fifo updated with spoilage -> PRODUCTION_ERROR mapping' as status;
