...
export default function ChartOfAccountsPage() {
  ...
  // State for Balance Recalculation
  const [isRecalcDialogOpen, setIsRecalcDialogOpen] = useState(false)
  const [isRecalculating, setIsRecalculating] = useState(false)
  const [recalcPreview, setRecalcPreview] = useState<{
    mismatchedAccounts: { id: string; code: string; name: string; type: string; currentBalance: number; expectedBalance: number; difference: number }[];
  ...
  // ============================================================================
  // RECALCULATE BALANCES FUNCTIONS
  // ============================================================================
  const handleOpenRecalcDialog = async () => {
    ...
    try {
      ...
      // For each account, calculate expected balance from journals
      const mismatchedAccounts: typeof recalcPreview['mismatchedAccounts'] = []

      for (const acc of branchAccounts || []) {
        // Get journal totals for this account
        // Note: We need to handle is_voided = NULL (not voided) and is_voided = false
        ...
        // Filter out voided journals (is_voided = true)
        // Keep: is_voided = false OR is_voided = NULL
        const validJournalData = (journalData || []).filter((l: any) => {
          const isVoided = l.journal_entries?.is_voided
          return isVoided !== true // Keep if false or null/undefined
        })

        const totalDebit = validJournalData.reduce((sum: number, l: any) => sum + (Number(l.debit_amount) || 0), 0)
        const totalCredit = validJournalData.reduce((sum: number, l: any) => sum + (Number(l.credit_amount) || 0), 0)

        // Calculate expected balance based on account type
        // Aset & Beban: normal debit, balance = initial + debit - credit
        // Kewajiban, Modal, Pendapatan: normal credit, balance = initial + credit - debit
        const initialBalance = Number(acc.initial_balance) || 0
        let expectedBalance: number

        if (['Aset', 'Beban'].includes(acc.type)) {
          expectedBalance = initialBalance + totalDebit - totalCredit
        } else {
          expectedBalance = initialBalance + totalCredit - totalDebit
        }
      ...
      mismatchedAccounts.sort((a, b) => Math.abs(b.difference) - Math.abs(a.difference))

      const totalMismatch = mismatchedAccounts.reduce((sum, a) => sum + Math.abs(a.difference), 0)

      setRecalcPreview({ mismatchedAccounts, totalMismatch })
    } catch (error: any) {
      console.error('Error loading recalc preview:', error)
      toast({
        variant: "destructive",
        title: "Error",
        description: "Gagal memuat data: " + error.message
  ...
  const handleRecalculateBalances = async () => {
    if (!currentBranch?.id || !recalcPreview || recalcPreview.mismatchedAccounts.length === 0) return
    ...
    try {
      ...
      for (const acc of recalcPreview.mismatchedAccounts) {
        ...
        if (error) {
          console.error(`Failed to update account ${acc.code}:`, error)
        } else {
          updatedCount++
        }
      }
      ...
      toast({
        title: "Sinkronisasi Berhasil",
        description: `${updatedCount} akun berhasil diupdate`
      })
  ...
  // Sinkronisasi cepat - langsung update semua akun branch ini tanpa preview
  const handleQuickSync = async () => {
    ...
    const confirmed = window.confirm(
      `Sinkronkan semua saldo akun untuk cabang "${currentBranch.name}"?\n\n` +
      `Ini akan menghitung ulang saldo semua akun berdasarkan jurnal yang sudah diposting.`
    )
    if (!confirmed) return
    ...
    try {
      ...
      for (const acc of branchAccounts || []) {
        ...
        // Filter out voided journals
        const validJournalData = (journalData || []).filter((l: any) => {
          const isVoided = l.journal_entries?.is_voided
          return isVoided !== true
        })
      ...
      toast({
        ...
        description: updatedCount > 0
          ? `${updatedCount} akun berhasil disinkronkan${errorCount > 0 ? `, ${errorCount} gagal` : ''}`
          : "Semua saldo sudah sinkron"
      })
      ...
      console.error('Error quick sync:', error)
      toast({
        variant: "destructive",
        title: "Sinkronisasi Gagal",
        description: error.message
  ...
  return (
    <div className="w-full max-w-none p-4 lg:p-6 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {userIsAdminOrOwner && (
          <div className="flex flex-wrap gap-2">
            {/* Tombol Quick Sync - sinkron langsung tanpa preview */}
            <Button
              onClick={handleQuickSync}
              disabled={isRecalculating}
              variant="default"
              title="Sinkronkan saldo semua akun berdasarkan jurnal"
            >
              {isRecalculating ? (
              ...
              {isRecalculating ? 'Menyinkronkan...' : 'Sinkron Saldo'}
            </Button>
            {/* Tombol Sinkron Saldo COA dengan preview - untuk lihat detail dulu */}
            <Button
              onClick={handleOpenRecalcDialog}
              disabled={isLoadingRecalc}
              variant="outline"
              title="Lihat preview akun yang tidak sinkron sebelum update"
            >
      ...
      <AlertDialog open={isRecalcDialogOpen} onOpenChange={setIsRecalcDialogOpen}>
        <AlertDialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <RefreshCw className="h-5 w-5" />
              Sinkronisasi Saldo COA
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-4">
                <p>
                  Fitur ini akan menghitung ulang saldo semua akun berdasarkan jurnal yang sudah diposting.
                  Gunakan jika ada ketidaksesuaian antara saldo akun dan jurnal.
                </p>

                {isLoadingRecalc && (
                  <div className="flex items-center justify-center py-8">
                    <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" />
                    <span className="ml-2">Memeriksa {accounts?.length || 0} akun...</span>
                  </div>
                )}

                {recalcPreview && recalcPreview.mismatchedAccounts.length === 0 && (
                  <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4 text-center">
                    <div className="text-green-600 font-semibold text-lg">✓ Semua Saldo Sinkron</div>
                    <p className="text-sm text-muted-foreground mt-1">
                      Tidak ada akun yang perlu diupdate. Semua saldo sudah sesuai dengan jurnal.
                    </p>
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-4">
                      <div className="flex items-center gap-2 text-amber-600 font-semibold">
                        ...
                        Ditemukan {recalcPreview.mismatchedAccounts.length} akun tidak sinkron
                      </div>
                      <p className="text-sm text-muted-foreground mt-1">
                        Total selisih: {formatCurrency(recalcPreview.totalMismatch)}
                      </p>
                    </div>
                    ...
                    <div className="border rounded-lg overflow-hidden">
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead className="text-right">Saldo Seharusnya</TableHead>
                            <TableHead className="text-right">Selisih</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {recalcPreview.mismatchedAccounts.slice(0, 20).map((acc) => (
                            <TableRow key={acc.id}>
                              ...
                              <TableCell className={cn(
                                ...
                                acc.difference > 0 ? "text-green-600" : "text-red-600"
                              )}>
                      ...
                      {recalcPreview.mismatchedAccounts.length > 20 && (
                        <div className="text-center text-sm text-muted-foreground py-2 border-t">
                          ...dan {recalcPreview.mismatchedAccounts.length - 20} akun lainnya
                        </div>
                      )}
                    </div>

                    <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-sm">
                      <p className="font-medium">Apa yang akan dilakukan:</p>
                      <ul className="list-disc list-inside mt-1 text-xs text-muted-foreground">
                        <li>Update saldo akun agar sesuai dengan jurnal yang sudah diposting</li>
                        <li>Tidak mengubah jurnal yang sudah ada</li>
                        <li>Memperhitungkan saldo awal (initial_balance)</li>
                      </ul>
                    </div>
                  </div>
                )}
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isRecalculating}>Batal</AlertDialogCancel>
            <AlertDialogAction
              onClick={handleRecalculateBalances}
              disabled={isRecalculating || !recalcPreview || recalcPreview.mismatchedAccounts.length === 0}
            >
              {isRecalculating ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                ...
                <>
                  <RefreshCw className="h-4 w-4 mr-2" />
                  Sinkronkan {recalcPreview?.mismatchedAccounts.length || 0} Akun
                </>
              )}
            </AlertDialogAction>
          </AlertDialogFooter>
