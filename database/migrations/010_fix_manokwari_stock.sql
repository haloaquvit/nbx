-- Migration 010: Fix Manokwari Stock and Inventory Journal
...
-- ============================================================================
-- STEP 2: CREATE INVENTORY BATCHES FOR PRODUCTS
-- ============================================================================
DO $$
DECLARE
  v_product RECORD;
...
-- ============================================================================
-- STEP 3: CREATE MATERIAL INVENTORY BATCHES
-- ============================================================================
DO $$
DECLARE
  v_material RECORD;
...
BEGIN
  ...
  FOR v_material IN
    ...
    WHERE m.stock > 0
      AND NOT EXISTS (
        SELECT 1 FROM material_inventory_batches mib
        WHERE mib.material_id = m.id
        AND mib.notes = 'Stok Awal'
      )
  LOOP
    -- Create batch with current stock
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_product_batch_count FROM inventory_batches WHERE notes = 'Stok Awal';
  SELECT COUNT(*) INTO v_material_batch_count FROM material_inventory_batches WHERE notes = 'Stok Awal';
