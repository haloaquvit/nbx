-- TEST PAYROLL FUNCTION
-- Script untuk test dan debug function calculate_payroll_with_advances

-- ========================================
-- STEP 1: CREATE SIMPLE TEST FUNCTION
-- ========================================

CREATE OR REPLACE FUNCTION public.test_calculate_payroll_with_advances(
  emp_id UUID,
  period_year INTEGER,
...
BEGIN
  -- Log parameters
  RAISE NOTICE 'Testing with parameters: emp_id=%, year=%, month=%', emp_id, period_year, period_month;
  ...
  -- Get active salary configuration (simple query)
  BEGIN
    SELECT
      es.id,
      es.base_salary,
    ...
    RAISE NOTICE 'Salary config found: %', salary_config;
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Error fetching salary config: %', SQLERRM;
    RETURN jsonb_build_object(
      'error', true,
      'message', 'Error fetching salary config: ' || SQLERRM,
      'employeeId', emp_id
    );
  END;
  ...
  RAISE NOTICE 'Base salary: %', base_salary;

  -- Simple commission calculation (from existing commission_entries if available)
  IF salary_config.payroll_type IN ('commission_only', 'mixed') THEN
    BEGIN
      ...
      RAISE NOTICE 'Commission amount: %', commission_amount;
    EXCEPTION WHEN OTHERS THEN
      commission_amount := 0;
      RAISE NOTICE 'Commission table not found or error: %', SQLERRM;
    END;
  END IF;

  -- Simple advance calculation (if table exists)
  BEGIN
    ...
    RAISE NOTICE 'Outstanding advances: %', outstanding_advances;
  EXCEPTION WHEN OTHERS THEN
    outstanding_advances := 0;
    RAISE NOTICE 'Advance tables not found or error: %', SQLERRM;
  END;
  ...
  RAISE NOTICE 'Final calculation: gross=%, deduction=%, net=%', gross_salary, total_deduction, net_salary;
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.test_calculate_payroll_with_advances(UUID, INTEGER, INTEGER) TO authenticated;

-- ========================================
-- STEP 2: REPLACE MAIN FUNCTION WITH TEST VERSION
-- ========================================

DROP FUNCTION IF EXISTS public.calculate_payroll_with_advances(UUID, INTEGER, INTEGER);
...
RETURNS JSONB AS $$
BEGIN
  -- Just call the test version for now
  RETURN public.test_calculate_payroll_with_advances(emp_id, period_year, period_month);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Test payroll function created with detailed logging!';
  RAISE NOTICE '🔧 Check the database logs for detailed debugging info';
  RAISE NOTICE '🧪 Try the RPC call from frontend now!';
END $$;