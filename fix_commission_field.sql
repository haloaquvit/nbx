-- FIX COMMISSION FIELD IN PAYROLL CALCULATION
-- Update function to use correct user_id field and proper date format
...
BEGIN
  ...
  -- FIXED: Commission calculation using correct user_id field and proper date range
  IF salary_config.payroll_type IN ('commission_only', 'mixed') THEN
    BEGIN
      SELECT COALESCE(SUM(amount), 0) INTO commission_amount
      FROM commission_entries ce
      WHERE ce.user_id = emp_id  -- CORRECT FIELD!
  ...
  -- Build success result with camelCase fields for frontend
...
DO $$
BEGIN
  RAISE NOTICE '✅ Fixed commission calculation!';
  RAISE NOTICE '🎯 Now using user_id field (not employee_id)';
  RAISE NOTICE '📅 Fixed date range format with proper timestamps';
  RAISE NOTICE '🧪 Test the payroll calculation now!';
END $$;